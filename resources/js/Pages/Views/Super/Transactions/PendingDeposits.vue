...
const openModal = (item) => {
  setSelected(item ?? props.deposits.data[0])
...
<template>
  <DashboardLayout>
    ...
    <div v-if="deposits.data !== null && parseInt(deposits.data.length) !== 0"
      ...
      <table id="leader_board_mobile" class="w-full border mb-[10px]">
        ...
        <tbody>
          <tr @click.prevent="openModal(item)" v-for="(item) in deposits.data">
            <td class="p-[3px] text-center">{{ item.id }}</td>
            <td class="p-[3px]">
              <p>Username: {{ item.account }}</p>
              <p>ID: {{ item.user_id }}</p>
            </td>
            ...
            <td>{{ item.amount }}</td>
            <td>
              <p class="mb-[10px]">Created At : {{ (new Date(item.created_at)).toString().split('GMT')[0].trim() }}</p>
              <p class="mb-[10px]">Updated At : {{ (new Date(item.updated_at)).toString().split('GMT')[0].trim() }}</p>
            </td>
    ...
    <div v-else>
      <p class="w-full flex items-center justify-center h-[400px] rounded font-bold text-[40px] bg-gray-200">No pending
        Deposits</p>
    </div>
  </DashboardLayout>
</template>
